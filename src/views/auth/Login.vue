<template>
  <div class="bg-food-icons2 h-screen flex items-center justify-center bg-black">
    <div class="absolute top-0 left-0 flex flex-col">
      <router-link :to="{name: 'restaurants.login'}">
        <Button link size="small" class="underline" label="Restaurant login" style="padding: 0;"/>
      </router-link>
      <router-link :to="{name: 'couriers.login'}">
        <Button link size="small" class="underline" label="Couriers login" style="padding: 0;"/>
      </router-link>
    </div>
    ...
    <div class="w-[700px] h-[500px] grid md:grid-cols-5">
      ...
      <div class="col-span-3 bg-surface-950 backdrop-blur-sm rounded-tr-lg rounded-br-lg flex flex-col">
        <h3 class="text-3xl text-center my-10 text-emerald-400">Login</h3>
...
const { handleLogin, getUser } = useAuthStore()
